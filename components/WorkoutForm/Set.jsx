  ...
  return (
    <div>
      <div className="flex justify-center items-center space-x-2 mb-2 w-full">
        <input type="number" name="weight" placeholder="Weight" className="p-2 border rounded w-full" value={workout.lifts[liftIndex].sets[setIndex].weight} onChange={(e) => handleWeightChange(liftIndex, setIndex, e.target.value)} required />
        <div className="">
          <XIcon height={20} width={20} color={'gray'} />
        </div>
        <input type="number" name="reps" placeholder="Reps" className="p-2 border rounded w-full" value={workout.lifts[liftIndex].sets[setIndex].reps} onChange={(e) => handleRepsChange(liftIndex, setIndex, e.target.value)} required />
